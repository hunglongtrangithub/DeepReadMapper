...
std::pair<const char*, size_t> read_fasta_default(const std::string& fasta_file, std::unique_ptr<char[]>& buffer) {
    ...
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"reading FASTA file"},
...
std::pair<const char*, size_t> read_fasta_mmap(const std::string& fasta_file, int& fd) {
    ...
    indicators::show_console_cursor(false);
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"mapping FASTA file (mmap)"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
...
std::string extract_FASTA_sequence(const std::string& fasta_file) {
    ...
    if (seq_start < data + data_size) seq_start++;  // Skip newline
...
std::pair<std::vector<std::string>, std::vector<size_t>> format_fasta(const char* data, size_t data_size,
    ...
    std::cout << "[FASTA] Estimated RAM usage: " << std::fixed << std::setprecision(2) << mem_usage << " MB"
              << std::endl;
    ...
    indicators::show_console_cursor(false);
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"processing FASTA windows"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
    ...
    std::cout << "[FASTA] Successfully processed " << result.size() << " windows from " << sequences.size()
              << " sequences" << std::endl;
...
std::pair<std::vector<std::string>, std::vector<size_t>> format_fasta_batch(
    ...
    for (const char* ptr = seq_start; ptr < data + data_size && result.size() < batch_size; ++ptr) {
        ...
        c = std::toupper(static_cast<unsigned char>(c));
        if (c != 'A' && c != 'T' && c != 'C' && c != 'G' && c != 'N') continue;
...
std::pair<std::vector<std::string>, std::vector<size_t>> format_fasta_mp(const char* data, size_t data_size,
    ...
    std::cout << "[FASTA] Processing " << descriptors.size() << " windows in " << num_batches << " batches"
              << std::endl;
    ...
    indicators::show_console_cursor(false);
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"processing FASTA windows (parallel)"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
    ...
    for (size_t batch_id = 0; batch_id < num_batches; ++batch_id) {
        ...
        for (size_t i = start_desc; i < end_desc; ++i) {
            ...
            while (seq_idx < sequences.size()) {
                ...
                if (i < cumulative_windows + seq_total_windows) break;
...
std::pair<const char*, size_t> read_fastq_default(const std::string& fastq_file, std::unique_ptr<char[]>& buffer) {
    ...
    indicators::show_console_cursor(false);
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"reading FASTQ file"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
...
std::pair<const char*, size_t> read_fastq_mmap(const std::string& fastq_file, int& fd) {
    ...
    indicators::show_console_cursor(false);
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"mapping FASTQ file (mmap)"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
...
std::pair<std::vector<std::string>, std::vector<std::string>> format_fastq(const char* data, size_t data_size,
    ...
    if (verbose) {
        ...
        progressBar = std::make_unique<indicators::ProgressBar>(
            indicators::option::BarWidth{80}, indicators::option::PrefixText{"processing FASTQ sequences"},
            indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true});
        progressBar->set_progress(0);
    }
