...
std::vector<std::tuple<std::vector<std::string>, std::vector<float>, std::vector<size_t>>> batch_reranker(
    ...
    std::cout << "[BATCH-RERANKER] Reranking " << total_queries << " queries with " << cand_embeddings.size()
              << " global embeddings (using indices)" << std::endl;

    // Pre-allocate results
    std::vector<std::tuple<std::vector<std::string>, std::vector<float>, std::vector<size_t>>> results(
        query_embeddings.size());

    // Progress bar
    indicators::show_console_cursor(false);
    indicators::ProgressBar rerankingBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"batch-reranking"},
