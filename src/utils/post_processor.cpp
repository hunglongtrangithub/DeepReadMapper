...
std::tuple<std::vector<std::string>, std::vector<size_t>, std::vector<size_t>> find_sequences(
    ...
    std::cout << "[FIND_SEQUENCES] Processing " << sparse_ids.size() << " sparse IDs with stride=" << stride
              << std::endl;
...
std::tuple<std::vector<std::string>, std::vector<int>, std::vector<size_t>> post_process_sw_dynamic(
    ...
    indicators::ProgressBar progressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"post-processing"},
...
#pragma omp parallel for num_threads(Config::Postprocess::NUM_THREADS) schedule(dynamic)
    for (size_t i = 0; i < total_queries; ++i) {
        ...
        std::vector<size_t> query_neighbors(converted_neighbors[i].begin(),
                                            converted_neighbors[i].begin() + num_cands_per_query);

        // Get candidate sequences using dynamic lookup (returns unique seqs + mapping)
        auto [unique_seqs, unique_ids, mapping_to_unique] =
            find_sequences(ref_genome, query_neighbors, ref_len, stride);
...
std::tuple<std::vector<std::string>, std::vector<float>, std::vector<size_t>> post_process_l2_dynamic(
    ...
    for (size_t i = 0; i < total_queries; ++i) {
        ...
        size_t num_cands_per_query = (stride == 1) ? std::min(k, converted_neighbors[i].size())
                                                   : std::min(k_clusters, converted_neighbors[i].size());
        sparse_ids.insert(sparse_ids.end(), converted_neighbors[i].begin(),
                          converted_neighbors[i].begin() + num_cands_per_query);
    }
    query_start_indices_sparse.push_back(sparse_ids.size());
    ...
    std::cout << "[POST-PROCESS] Vectorizing " << unique_seqs.size() << " unique candidates (deduplicated from "
              << mapping_to_unique.size() << " total expansions)." << std::endl;
    start_time = std::chrono::high_resolution_clock::now();
    ...
    for (size_t chunk_start = 0; chunk_start < unique_seqs.size(); chunk_start += CHUNK_SIZE) {
        ...
        std::vector<std::string> chunk(unique_seqs.begin() + chunk_start, unique_seqs.begin() + chunk_end);
        std::vector<std::vector<float>> chunk_embeddings = vectorizer.vectorize(chunk, false);
        unique_embeddings.insert(unique_embeddings.end(), chunk_embeddings.begin(), chunk_embeddings.end());
    }
    ...
    auto batch_results = batch_reranker(expanded_seqs, expanded_ids, expand_embedding_ids, unique_embeddings,
                                        query_start_indices_expanded, query_embeddings, k);
...
void post_process_l2_dynamic_streaming(const std::vector<std::vector<NeighborType>> &neighbors,
    ...
    if (stride == 1) {
        ...
        indicators::ProgressBar streamBar{
            indicators::option::BarWidth{80}, indicators::option::PrefixText{"streaming results"},
            indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
        ...
        for (size_t batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
            ...
            write_sam_streaming(batch_seqs, batch_scores, query_seqs, query_ids, batch_ids, ref_name, ref_len, k,
                                sam_file, batch_start, write_header, batch_size);
    ...
    std::cout << "[POST-PROCESS] Reranking and streaming " << total_queries << " queries in " << num_batches
              << " batches (size: " << QUERY_BATCH_SIZE << ")" << std::endl;

    indicators::show_console_cursor(false);
    indicators::ProgressBar batchProgressBar{
        indicators::option::BarWidth{80}, indicators::option::PrefixText{"reranking & streaming batches"},
        indicators::option::ShowElapsedTime{true}, indicators::option::ShowRemainingTime{true}};
    ...
    for (size_t batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
        ...
        std::vector<std::vector<float>> batch_query_embeddings(query_embeddings.begin() + batch_start,
        ...
        // Rerank batch
        auto batch_results = batch_reranker(batch_seqs, batch_ids, batch_embedding_indices, unique_embeddings,
                                            batch_query_boundaries, batch_query_embeddings, k);
        ...
        bool write_header = (batch_idx == 0);
        write_sam_streaming(result_seqs, result_scores, query_seqs, query_ids, result_ids, ref_name, ref_len, k,
                            sam_file, batch_start, write_header);
...
std::tuple<std::vector<std::string>, std::vector<float>, std::vector<size_t>> post_process_l2_static(
    ...
    std::cout << "[POST-PROCESS] Collected " << all_neighbor_ids.size() << " neighbor IDs from " << neighbors.size()
              << " queries" << std::endl;
    ...
    std::cout << "[POST-PROCESS] Vectorizing " << unique_seqs.size() << " unique candidates (deduplicated from "
              << total_expansions << " total expansions)." << std::endl;
    std::vector<std::vector<float>> unique_embeddings = vectorizer.vectorize(unique_seqs, false);
    end_time = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
    std::cout << "[POST-PROCESS] Vectorization completed in " << duration.count() << " ms" << std::endl;
    std::cout << "[POST-PROCESS] Saved " << (total_expansions - unique_seqs.size()) << " redundant vectorizations!"
              << std::endl;
    ...
    start_time = std::chrono::high_resolution_clock::now();
    std::cout << "[POST-PROCESS] Running batched reranker with pre-computed embeddings." << std::endl;
    auto results = batch_reranker(cand_seqs, expand_cand_ids, cand_embedding_ids, unique_embeddings,
                                  query_start_indices, query_embeddings, k_clusters);
