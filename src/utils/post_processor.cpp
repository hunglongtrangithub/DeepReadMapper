...
std::vector<std::string> find_sequences(const std::vector<std::string> &ref_seqs, const std::vector<size_t> &ids, size_t stride)
{
    ...
    for (size_t i = 0; i < sorted_ids.size(); ++i)
    {
        ...
        size_t start = (actual_position >= stride - 1) ? actual_position - stride + 1 : 0;
        size_t end = std::min(actual_position + stride, ref_seqs.size()); // FIX: Add bounds checking
...
// Helper function to convert neighbor types
template<typename NeighborType>
    ...
    size_t ref_len, size_t stride, size_t k)
{
    ...
    return post_process_core<std::string, int>(
        converted_neighbors, distances, ref_genome, query_seqs, ref_len, stride, k,
        [](const std::vector<std::string>& cand_seqs, const std::string& query_seq, size_t k) {
    ...
    Vectorizer& vectorizer)
{
    ...
    for (size_t i = 0; i < total_queries; ++i)
    {
        ...
        size_t num_cands_per_query = std::min(size_t(5), converted_neighbors[i].size());
    ...
    Vectorizer& vectorizer)
{
    ...
    if (stride == 1)
    {
        std::cout << "[POST-PROCESS] k >= num_candidates, skipping reranker (early termination)" << std::endl;
        
        // Create dummy L2 distances (all zeros or use HNSW distances)
        std::vector<std::string> final_seqs;
        std::vector<float> final_scores;
        final_seqs.reserve(total_queries * k);
        final_scores.reserve(total_queries * k);
        ...
        for (size_t i = 0; i < total_queries; ++i) {
            ...
            for (size_t j = 0; j < std::min(k, actual_cands) && seq_idx < all_cand_seqs.size(); ++j, ++seq_idx) {
                ...
                final_scores.push_back(0.0f); // Dummy score, or use distances[i][j] if available
            }
            
            // Pad with empty strings if needed
            for (size_t j = actual_cands; j < k; ++j) {
                final_seqs.push_back("");
                final_scores.push_back(std::numeric_limits<float>::max());
            }
        }
...
template std::pair<std::vector<std::string>, std::vector<int>> post_process_sw<size_t>(const std::vector<std::vector<size_t>>&, const std::vector<std::vector<float>>&, const std::string&, const std::vector<std::string>&, size_t, size_t, size_t);
template std::pair<std::vector<std::string>, std::vector<int>> post_process_sw<long int>(const std::vector<std::vector<long int>>&, const std::vector<std::vector<float>>&, const std::string&, const std::vector<std::string>&, size_t, size_t, size_t);

template std::pair<std::vector<std::string>, std::vector<float>> post_process_l2_dynamic<size_t>(const std::vector<std::vector<size_t>>&, const std::vector<std::vector<float>>&, const std::string&, const std::vector<std::string>&, size_t, size_t, size_t, const std::vector<std::vector<float>>&, Vectorizer&);
template std::pair<std::vector<std::string>, std::vector<float>> post_process_l2_dynamic<long int>(const std::vector<std::vector<long int>>&, const std::vector<std::vector<float>>&, const std::string&, const std::vector<std::string>&, size_t, size_t, size_t, const std::vector<std::vector<float>>&, Vectorizer&);

template std::pair<std::vector<std::string>, std::vector<float>> post_process_l2_static<size_t>(const std::vector<std::vector<size_t>>&, const std::vector<std::vector<float>>&, const std::vector<std::string>&, const std::vector<std::string>&, size_t, size_t, size_t, const std::vector<std::vector<float>>&, Vectorizer&);
template std::pair<std::vector<std::string>, std::vector<float>> post_process_l2_static<long int>(const std::vector<std::vector<long int>>&, const std::vector<std::vector<float>>&, const std::vector<std::string>&, const std::vector<std::string>&, size_t, size_t, size_t, const std::vector<std::vector<float>>&, Vectorizer&);