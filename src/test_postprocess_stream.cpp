...
int main() {
    try {
        ...
        std::cout << "=== DeepAligner Post-Processing STREAMING Test ===" << std::endl << std::endl;
        ...
        const std::string query_seqs_file =
            "/mnt/4T-Samsung1/PacBio_realdata/fastq/HG002_PacBio-HiFi-Revio_20231031_48x_GRCh38-GIABv3.fastq";
        ...
        std::cout << "[TEST] Reference name: " << ref_name << std::endl << std::endl;
        ...
        end_time = std::chrono::high_resolution_clock::now();
        duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
        std::cout << "[TEST] Vectorizer initialization time: " << duration.count() << " ms" << std::endl << std::endl;
        ...
        std::cout << "[TEST] Query embeddings: " << query_embeddings.size() << " x " << query_embeddings[0].size()
                  << std::endl;
        std::cout << "[TEST] Vectorization time: " << duration.count() << " ms" << std::endl << std::endl;
        ...
        auto [ref_sequences, dense_ids, mapping_indices] =
            find_sequences(ref_genome, unique_sparse_ids, ref_len, stride);
        ...
        end_time = std::chrono::high_resolution_clock::now();
        duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
        std::cout << "[TEST] Reference embeddings: " << ref_embeddings.size() << " x " << ref_embeddings[0].size()
                  << std::endl;
        std::cout << "[TEST] Step 3 time: " << duration.count() << " ms (" << (duration.count() / 1000.0) << " seconds)"
                  << std::endl
                  << std::endl;
