...
void write_npy_header(std::ofstream &file, size_t rows, size_t cols) {
    ...
    header_stream << "{'descr': '<f4', 'fortran_order': False, 'shape': (" << rows << ", " << cols << "), }";
...
int main(int argc, char *argv[]) {
    ...
    if (file_ext == ".fasta" || file_ext == ".fna" || file_ext == ".fa") {
        ...
        for (const char *ptr = seq_start; ptr < data + data_size; ++ptr) {
            ...
            if (c == 'A' || c == 'T' || c == 'C' || c == 'G' || c == 'N') genome_length++;
        }
        ...
        std::cout << "[INFO] Output file size: ~"
                  << (estimated_total_sequences * 128 * sizeof(float) / (1024.0 * 1024.0 * 1024.0)) << " GB"
                  << std::endl
                  << std::endl;
        ...
        while (!is_complete) {
            ...
            auto [batch_sequences, batch_labels] =
                format_fasta_batch(data, data_size, sequences_file, ref_len, 1, false, batch_size, resume_pos,
                                   position_counter, buffer_state, buf_start_state, is_complete);
        ...
        if (!all_embeddings.empty()) {
            ...
            cnpy::npy_save(output_file, &flattened_embeddings[0], {num_sequences, embedding_dim});
