...
void enableProfiling() { _Profiling_enabled = true; }

void disableProfiling() { _Profiling_enabled = false; }

uint64_t getCountDistCalc() { return _Count_dist_calc; }

void resetProfilingCounter() { _Count_dist_calc = 0; }
...
uint32_t NSW::getNeighborhoodSize(nodeID_t nodeID) {
    ...
    if (!neighborhood.has_value()) throw std::runtime_error("asked for neighborhood size but node does not exist!");
    return this->neighbors_[nodeID]->size();
}

const std::vector<nodeID_t> &NSW::getNeighborsId(nodeID_t nodeID) const { return *(this->neighbors_[nodeID]); }

bool NSW::isEmpty() const { return this->numNodes_ == 0; }

uint32_t NSW::getNumNodes() const { return this->numNodes_; }

bool NSW::containsNode(nodeID_t nodeID) const { return this->neighbors_[nodeID] != std::nullopt; }
...
void NSW::addNode(uint32_t nodeID, const std::vector<uint32_t> &neighborsNodeIds) {
    ...
    if (this->containsNode(nodeID)) {
        throw std::runtime_error("nodeID " + std::to_string(nodeID) +
                                 " already exist in this NSW graph, can't add again");
    }
...
void NSW::removeConnection(nodeID_t nodeID1, nodeID_t nodeID2) {
    ...
    auto postRemoveIter = std::remove(this->neighbors_[nodeID1]->begin(), this->neighbors_[nodeID1]->end(), nodeID2);
    this->neighbors_[nodeID1]->erase(postRemoveIter, this->neighbors_[nodeID1]->end());
    postRemoveIter = std::remove(this->neighbors_[nodeID2]->begin(), this->neighbors_[nodeID2]->end(), nodeID1);
...
void NSW::summarize() const {
    ...
    if (!(this->isEmpty())) {
        ...
        for (const auto &neighborList : this->neighbors_) {
            if (!neighborList.has_value()) continue;
...
void NSW::printNodes() const {
    if (this->numNodes_ < 20) {
        ...
        for (nodeID_t nodeID = 0; nodeID < maxNumNodes_; nodeID++) {
            if (this->containsNode(nodeID)) std::cout << nodeID << " ";
        }
    } else {
        // print first 10
...
const std::vector<float> &HNSW::getVector_(nodeID_t nodeID) const { return this->vectors_[nodeID]; }
...
void NSW::findAndSetDefaultEntry() {
    ...
    for (nodeID_t i = 0; i < this->maxNumNodes_; i++) {
        if (!this->containsNode(i)) continue;
...
std::vector<uint32_t> HNSW::calculateNumNodesInLayers_(uint32_t total) const {
    double mL = this->mL_;
    auto cdf = [mL](double x) { return 1 - std::exp(-x / mL); };
    ...
    expected.erase(std::remove_if(expected.begin(), expected.end(), [](uint32_t e) { return e == 0; }), expected.end());
...
void HNSW::selectNeighborsSimple_(const std::vector<float> &query, std::vector<search_result_t> &candidates,
                                  uint32_t M) const {
    candidates.resize(M);
    return;
}

void HNSW::selectNeighborsHeuristic_(const std::vector<float> &query, std::vector<search_result_t> &candidates,
                                     uint32_t M, uint32_t layerID) {
...
void precomputeDistances(const std::vector<float> &query, const std::vector<std::vector<float>> &db,
                         const std::vector<nodeID_t> &neighborIDs, VisitedCheck &visited, std::vector<float> &outDist) {
    ...
    for (unsigned i = 0; i < neighborIDs.size(); i++) {
        if (i % 16 == 0) _mm_prefetch(reinterpret_cast<const char *>(&outDist[i + 16]), _MM_HINT_T0);
...
std::vector<search_result_t> HNSW::searchLayer(uint32_t layerID, nodeID_t entrypointID, const std::vector<float> &query,
                                               uint32_t ef) {
    const NSW &layerGraph = this->layers[layerID];
    if (layerGraph.isEmpty()) return std::vector<search_result_t>(0);
    if (entrypointID == ENTRYPOINT_NOT_SET) entrypointID = layerGraph.getEntrypointID();
    ...
    while (!candidates.empty()) {
        ...
        if (candidateDist > nearestNeighbors.back().first) break;
        ...
        for (unsigned i = 0; i < neighborIDs.size(); i++) {
            float neighborDist;
            if (i % 16 == 12) _mm_prefetch(reinterpret_cast<const char *>(&this->searchDists_[i + 16]), _MM_HINT_T0);
            ...
            if ((neighborDist < nearestNeighbors.back().first) || nearestNeighbors.size() < ef) {
                ...
                if (nearestNeighbors.size() > ef) nearestNeighbors.pop_back();
            }
        }
    }
...
std::vector<search_result_t> HNSW::search(const std::vector<float> &query, uint32_t ef) {
    ...
    if (query.size() != this->vectors_[0].size()) throw std::invalid_argument("query does not have correct dimensions");
...
std::vector<std::vector<search_result_t>> HNSW::searchParallel(const std::vector<std::vector<float>> &queries,
                                                               uint32_t ef) {
    if (this->layers[0].isEmpty()) {
        return {};
    }
...
uint32_t HNSW::getNumLayers() {
    ...
    for (uint32_t i = 0; i < this->layers.size(); i++) {
        if (this->layers[i].isEmpty()) break;
        n = i;
    }
    this->nLayers_ = n;
    return n;
...
void HNSW::buildIndex(const std::vector<std::vector<float>> &vectors) {
    ...
    // This array contains partition endpoints (a, b), (b, c) where a, b, c are index on vectors. Endpoints are excluded
    // from partitions
    ...
    for (int layerID = layerSizes.size() - 1; layerID >= 0; --layerID) {
        ...
        indicators::ProgressBar progressBar{
            ...
            indicators::option::PrefixText{"layer " + std::to_string(layerID) + " adding " + std::to_string(nPoints) +
                                           " nodes"},
...
void HNSW::summarize() const {
    ...
    for (int layerID = this->layers.size() - 1; layerID >= 0; layerID--) {
        ...
        if (layerGraph.isEmpty()) continue;
