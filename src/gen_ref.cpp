...
int main(int argc, char *argv[]) {
    ...
    app.add_option("-i,--input", input_file, "Input FASTA or FASTQ file")->required()->check(CLI::ExistingFile);

    // Optional arguments with defaults
    int ref_len = 0;
    app.add_option("-l,--ref-len", ref_len, "Reference length (FASTA only)")->check(CLI::NonNegativeNumber);

    size_t stride = 1;
    app.add_option("-s,--stride", stride, "Stride between windows (FASTA only)")->check(CLI::PositiveNumber);
    ...
    try {
        ...
        auto [sequences, query_ids] = read_file(input_file,
        ...
        indicators::ProgressBar progressBar{
            indicators::option::BarWidth{80}, indicators::option::PrefixText{"writing sequences"},
        ...
        if (is_fasta) std::cout << "Total windows: " << sequences.size() << std::endl;
