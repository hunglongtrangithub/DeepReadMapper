...
class HNSW {
   private:
    ...
    void selectNeighborsSimple_(const std::vector<float> &query, std::vector<search_result_t> &candidates,
                                uint32_t M) const;
    // select M points from candidates to connect to query
    // Return: List of selected (distance_to_query, nodeID) from candidates, modified in-place
    // assume candidates are already sorted by distance
    void selectNeighborsHeuristic_(const std::vector<float> &query, std::vector<search_result_t> &candidates,
   ...
   public:
    ...
    std::vector<search_result_t> searchLayer(uint32_t layerID, nodeID_t entrypointID, const std::vector<float> &query,
                                             uint32_t ef);
    ...
    std::vector<std::vector<search_result_t>> searchParallel(const std::vector<std::vector<float>> &queries,
                                                             uint32_t ef);
