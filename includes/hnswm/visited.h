...
class VisitedCheck {
   ...
   public:
    ...
    void clear() { this->curCounterVal_++; }

    // mark visited
    void insert(uint32_t x) {
        if (x >= this->size_) throw std::runtime_error("Visited set was given a value larger than what was allocated");
        this->data_[x] = this->curCounterVal_;
    }

    // check if visited
    bool contains(uint32_t x) { return (this->data_[x] == this->curCounterVal_); }
