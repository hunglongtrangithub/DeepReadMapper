...
constexpr const int CHUNK_SIZE =
    1000;  // Number of sequences per chunk during preprocessing. 1 thread handles 1 chunk at a time.
}  // namespace Preprocess

namespace Inference {
constexpr const char MODEL_PATH[] = "models/finetuned_sgn33-new-a-Apr6.xml";
// constexpr const char MODEL_PATH[] =
// "/home/tam/tam-workspace/Research/Optimized-DeepAligner-CPU/models/finetuned_sgn33-new-a-Apr6.xml"; // Path to the
// model file
constexpr const size_t BATCH_SIZE = 100;  // Max num of sequences/batch. Actual batch may be smaller.
constexpr const size_t MAX_LEN =
    123;  // Max sequence length handled by the model. If shorter or longer, will be padded/truncated.
...
constexpr const size_t NUM_INFER_REQUESTS =
    2048;  // Max num of inference requests handled concurrently (1 request = 1 batch)
}  // namespace Inference
...
constexpr const int K_CLUSTERS = 5;  // [Sparse Index] Number of clusters (sparse candidates) to search for each query,
                                     // K <= K_CLUSTERS * 2 * stride & K_CLUSTERS <= EF
constexpr const int NUM_THREADS = 128;  // Number of threads for parallel search
}  // namespace Search
