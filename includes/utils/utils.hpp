...
std::pair<std::vector<std::string>, std::vector<std::string>> read_file(const std::string& file_path,
                                                                        size_t ref_len = 150, size_t stride = 1,
...
int save_results(const std::vector<std::vector<size_t>>& neighbors, const std::vector<std::vector<float>>& distances,
                 const std::string& indices_file, const std::string& distances_file, size_t k,
                 const bool use_npy = true);
...
void write_sam_streaming(const std::vector<std::string>& cand_seqs, const std::vector<float>& scores,
                         ...
                         const std::string& sam_file, size_t query_offset = 0, bool write_header = false,
...
/// @return 0 if successful.
int save_id_map(const std::vector<size_t>& labels, const std::string& folder_path,
                const std::string& mapping_file = "id_map.bin");
